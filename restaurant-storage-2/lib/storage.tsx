...
export function StorageProvider({ children }: { children: React.ReactNode }) {
  ...
  const increaseQuantity = useCallback((id: string, amount: number, employeeName: string) => {
    setItems((prev) => {
      const item = prev.find((i) => i.id === id)
      if (item) {
  ...
  const decreaseQuantity = useCallback((id: string, amount: number, employeeName: string) => {
    setItems((prev) => {
      const item = prev.find((i) => i.id === id)
      if (item) {
  ...
  const contextValue = useMemo(
    ...
    [items, activities, addItem, updateItem, removeItem, addActivity, increaseQuantity, decreaseQuantity],
  )
